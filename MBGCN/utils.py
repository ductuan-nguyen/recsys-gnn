...
from model_orig import MF, MBGCN
...
class ContentManager(object):
    ...
    def model_save(self, model, epoch=None):
        if epoch is not None:
            model_name = 'model_epoch_{}.pkl'.format(epoch)
        else:
            model_name = 'model.pkl'
        torch.save(model.state_dict(), os.path.join(self.path, model_name))

    def model_load(self, model: MBGCN, ckpt_path):
...
class ModelSelector(object):
    ...
    def getModel(flags_obj, dataset, model_name, device):
        if model_name=='MF':
            ...
            # model = MBGCN(flags_obj, dataset, device)
            # ## not train some parameters
            # model.item_embedding.requires_grad = False
            # model.user_embedding.requires_grad = False
            # return model
