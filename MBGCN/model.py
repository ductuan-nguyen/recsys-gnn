    ...
    def __init__(self, flags_obj, trainset, device):
        ...
        if flags_obj.create_embeddings=='True':
            ...
            if flags_obj.pretrain_frozen==False:
                ...
                # name_embeds = products['name'].apply(lambda x: np.fromstring(x[1:-1], sep=' ')).tolist()
                # name_embeds = torch.tensor(name_embeds, dtype=torch.float) / 255.0
                category = torch.tensor(products['category'].values, dtype=torch.long)
                price = torch.tensor(products['price'].values, dtype=torch.long)
                self.cat_embed = nn.Embedding(max(category.max(), 1) + 1, 16)
                self.price_embed = nn.Embedding(max(price.max(), 1) + 1, 16)

                # item_embedding = F.normalize(load_data['item_embedding'])
                item_embedding = load_data['item_embedding']
                item_embedding = F.normalize(item_embedding, dim=1)
                print('Stats of item embedding:', item_embedding.shape, item_embedding.min(), item_embedding.max())
                cat_embed = self.cat_embed(category)
                print('Stats of category embedding:', cat_embed.shape, cat_embed.min(), cat_embed.max())
                price_embed = self.price_embed(price)
                print('Stats of price embedding:', price_embed.shape, price_embed.min(), price_embed.max())
                name_embeds = products['name'].apply(lambda x: np.fromstring(x[1:-1], sep=' ')).tolist()
                name_embeds = torch.tensor(name_embeds, dtype=torch.float)
                # norm to (-1, 1)
                name_embeds = F.normalize(name_embeds, dim=1)
                print('Stats of name embedding:', name_embeds.shape, name_embeds.min(), name_embeds.max())
                name_embeds = F.normalize(name_embeds, dim=1)
                print('Stats of name embedding after normalization:', name_embeds.shape, name_embeds.min(), name_embeds.max())
                item_embedding = torch.cat((item_embedding, self.cat_embed(category), self.price_embed(price), name_embeds), dim=1)
                # item_embedding = F.normalize(item_embedding, dim=1)
                self.item_embedding = nn.Parameter(item_embedding.to(self.device))

                user_embedding = load_data['user_embedding']
                print('Stats of user embedding:', user_embedding.shape, user_embedding.min(), user_embedding.max())
                handcrafted_features = np.load('/home/tuannd/recsys2025/embed-dir/embeddings.npy')
                handcrafted_features = torch.tensor(handcrafted_features, dtype=torch.float)
                print('Stats of handcrafted features:', handcrafted_features.shape, handcrafted_features.min(), handcrafted_features.max())
                handcrafted_features = F.normalize(handcrafted_features, dim=1)
                # normalize to (-1, 1)
                print('Stats of handcrafted features after normalization:', handcrafted_features.shape, handcrafted_features.min(), handcrafted_features.max())
                
                user_embedding = torch.cat((user_embedding, handcrafted_features), dim=1)
                # user_embedding = F.normalize(user_embedding, dim=1)

                self.user_embedding = nn.Parameter(user_embedding.to(self.device))
...
class MBGCN(model_base):
    ...
    def forward(self, user, item):
        ...
        for i, key in enumerate(self.relation_dict):
            ...
            tmp_item_propagation = torch.mm(torch.mm(self.item_graph[key].float(), self.item_embedding) / result, self.item_propagate_W[i])
