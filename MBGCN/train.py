...
class TrainManager(object):
    ...
    def data_set_init(self, flags_obj):
        ...
        print(len(self.validationset), self.validationset[0][1].shape)
        ...
        print('DataLoader Read Completed!')
    def train(self):
        self.set_leaderboard()

        for epoch in range(self.flags_obj.epoch):
            print(self.model.item_embedding.requires_grad)
            ...
            self.cm.model_save(self.model, epoch=epoch)
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(tqdm(self.validationloader)):
                ...
                users, ground_truth = data
                # print(users.shape, ground_truth.shape, train_mask.shape)
    ...
    def test(self):
        best_model = ModelSelector.getModel(self.flags_obj, self.trainset, 'MBGCN', device=self.device)
        self.model = self.cm.model_load(best_model, self.flags_obj.trained_mbgcn_model)

        self.model.eval().to(self.device)
        with torch.no_grad():
            user_features  = self.model.get_all_user_feature()
        print('User features shape:', user_features.shape)
        # save 
        user_features = user_features.cpu().numpy().astype(np.float16)
        if not os.path.exists(self.flags_obj.output_test):
            os.makedirs(self.flags_obj.output_test)
        np.save(os.path.join(self.flags_obj.output_test, 'embeddings.npy'), user_features)
        import subprocess
        command = f'cp submits/client_ids.npy {self.flags_obj.output_test}'
